...
function OvInner() {
  ...
  useEffect(() => {
    (async () => {
      ...
      for (let i = 0; i < ids.length; i += 200) {
        const r = await supabase.from("item_grid").select("account_id,product_key,item_name,l90,l90_prev,l52").in("account_id", ids.slice(i, i + 200));
  ...
  const title = useMemo(() => {
    ...
    if (scope.city) return `${titleCase(scope.city)}, ${scope.st || ""}`.replace(/, $/, "");
    if (scope.channel && scope.st) return `${titleCase(scope.channel)} · ${STNAME[scope.st] || scope.st}`;
    if (scope.channel) return titleCase(scope.channel);
  ...
  const plcByAcct = useMemo(() => {
    if (!grid) return null;
    const map = {};
    for (const it of grid) if ((it.l90 || 0) > 0) map[it.account_id] = (map[it.account_id] || 0) + 1;
    return map;
  }, [grid]);
  ...
  const health = useMemo(() => {
    ...
    for (const a of rows) { const k = healthBucket(a.headline); g[k].n++; g[k].vol += vol(a); }
  ...
  const items = useMemo(() => {
    ...
    for (const it of grid) {
      const e = byItem[it.product_key || it.item_name] || (byItem[it.product_key || it.item_name] = { name: it.item_name, l90: 0, prev: 0, doorsNow: 0, doorsPrev: 0 });
  ...
  // scope-aware breakdown dimensions: always two meaningful tables
  const dims = useMemo(() => {
  ...
  function buildDim(dim) {
    if (!rows || !plcByAcct) return null;
    if (dim === "channel") {
      const totL52 = rows.reduce((s, a) => s + (a.account_weight || 0), 0) || 1;
      ...
      const fin = e => ({ ...e, ros: e.accts ? e.cases / e.accts / 3 : 0, avgPlc: e.accts ? e.plc / e.accts : 0, gPct: gpct(e.cases, e.prev) });
      const out = big.map(r => ({ ...r, label: titleCase(r.key) }));
      if (smallOff.accts) out.push({ ...fin(smallOff), label: "All other off" });
      if (smallOn.accts) out.push({ ...fin(smallOn), label: "All other on" });
      return { kind: "channel", lastCol: "ROS", rows: out };
    }
    if (dim === "chain") {
      const r = aggBy(a => a.chain ? a.chain : "__indie__").sort((a, b) => b.cases - a.cases);
      const indie = r.find(x => x.key === "__indie__");
      const chains = r.filter(x => x.key !== "__indie__").slice(0, 6).map(x => ({ ...x, label: titleCase(x.key) }));
      if (indie) chains.push({ ...indie, label: "Independents", isIndie: true });
      return { kind: "chain", lastCol: "plc/ac", rows: chains };
    }
    if (dim === "city") {
      const r = aggBy(a => a.city || null).sort((a, b) => b.cases - a.cases).slice(0, 7).map(x => ({ ...x, label: titleCase(x.key) }));
      return { kind: "city", lastCol: "ROS", rows: r };
    }
    return null;
  }
  ...
  const summary = useMemo(() => {
    ...
    if (m.pct != null && m.acctPct != null && m.pct >= 3 && (m.pct - m.acctPct) <= -4) opp.push({ mag: Math.abs(m.pct - m.acctPct) + 5, t: "New accounts haven't ramped",
      d: `${health.new.n} new accounts already make up ${health.new.pct}% of volume and haven't hit full stride yet — that upside is banked, not yet earned. Lock them in with a second SKU or an intro promo before they drift. New-account momentum is the cheapest growth on this page.` });   // profile-literal-ok — prose in a narrative sentence, not a label
        ...
        {m && (
          <div className="nobar" style={{ flex: 1, overflowY: "auto", padding: "0 16px 40px", WebkitOverflowScrolling: "touch" }}>
            ...
            <section>
              ...
              {health && (
                <>
                  <div style={{ fontSize: 11, color: "var(--text-3)", marginTop: 3 }}>{m.n.toLocaleString()} accounts · circle size = share of 90-day volume</div>
                  <HealthCircles health={health} scope={scope} router={router} />
                </>
              )}
            </section>

            <section>
              <SecTag n="04" label="Where It Sells" />
              <H1>{dims[0] === "channel" ? "Channel" : titleCase(dims[0])} & {titleCase(dims[1])}</H1>
              <div style={{ fontSize: 11, fontWeight: 700, color: "var(--text-2)", marginTop: 12, marginBottom: 6 }}>BY {dims[0].toUpperCase()}</div>
              {tableA && <BreakdownTable table={tableA} scope={scope} router={router} />}
              <div style={{ fontSize: 11, fontWeight: 700, color: "var(--text-2)", marginTop: 16, marginBottom: 6 }}>BY {dims[1].toUpperCase()}</div>
              {tableB && <BreakdownTable table={tableB} scope={scope} router={router} />}
            </section>
...
function PrintDeck({ deckRef, title, kind, m, health, items, movers, verdict, tableA, tableB, dims, summary, demoRows, bookLists }) {
  ...
  const allRows = [...tableA.rows, ...tableB.rows];
  const topGrp = [...allRows].filter(r => !String(r.key).startsWith("All other") && r.gPct != null).sort((a, b) => b.gPct - a.gPct)[0];
  const softGrp = [...allRows].filter(r => !String(r.key).startsWith("All other") && r.gPct != null).sort((a, b) => a.gPct - b.gPct)[0];
    ...
    <div className="print-deck" ref={deckRef}>
      ...
      <div className="pslide">
        ...
        <div style={{ flex: 1, padding: "0.2in 0.34in", display: "flex", flexDirection: "column", gap: 12, minHeight: 0 }}>
          <div style={{ display: "flex", gap: 16, flex: 1, minHeight: 0 }}>
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
              <PPanelHead>By {dims[1]}</PPanelHead>
              <PTable table={tableB} />
            </div>
          </div>
      ...
      <div className="pslide">
        ...
        <div style={{ flex: 1, padding: "0.2in 0.34in", display: "flex", flexDirection: "column", minHeight: 0 }}>
          ...
          <div style={{ fontSize: 10, color: PT.mut, marginTop: 10, textAlign: "center" }}>Prepared by {PREPARED_BY} · {title} · data thru {DATA_THRU}</div>
        </div>
        <PFoot page="7" />
      </div>
    </div>
...
function PTable({ table }) {
  const rows = table.rows, lastCol = table.lastCol, kind = table.kind;
    ...
    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12.5 }}>
      <thead>
        <tr>
          ...
          <th style={hd}>90D cs</th><th style={hd}>Δ%</th><th style={hd}>acct</th><th style={hd}>{lastCol}</th>
        </tr>
      </thead>
      <tbody>
        {rows.map(r => {
          ...
          return (
            <tr key={r.key} style={{ background: bg }}>
              ...
              <td style={{ ...cell, borderBottom: `1px solid ${PT.line}` }}>{lastCol === "plc/ac" ? r.avgPlc.toFixed(1) : r.ros.toFixed(1)}</td>
            </tr>
          );
        })}
      </tbody>
        ...
        {order.map(o => {
          ...
          return (
            <div key={o.key} onClick={() => go(o.key)} title={`View ${o.lab} accounts`} role="button" tabIndex={0}
              ...
              <div style={{ fontSize: 11, fontWeight: 700, color: o.ink, marginTop: 8, textAlign: "center", whiteSpace: "nowrap" }}>{o.lab}</div>
...
function BreakdownTable({ table, scope, router }) {
  const rows = table.rows, kind = table.kind, lastCol = table.lastCol;
  const go = r => {
    const p = new URLSearchParams();
    if (scope.st) p.set("st", scope.st);
    if (scope.city) p.set("city", scope.city);
    if (scope.channel) p.set("channel", scope.channel);
    if (scope.chain) p.set("chain", scope.chain);
    if (String(r.key).startsWith("All other")) return;
    if (kind === "channel") p.set("channel", r.key);
    else if (kind === "chain") { if (r.isIndie) return; p.set("chain", r.key); }
    else if (kind === "city") p.set("city", r.key);
  ...
  return (
    <div style={{ background: "var(--surface)", border: "0.5px solid var(--border)", borderRadius: "var(--r-md)", boxShadow: "var(--shadow)", overflow: "hidden" }}>
      <div style={{ display: "flex", borderBottom: "1px solid var(--border-strong)" }}>
        ...
        <span style={{ ...head, width: 34, ...col, paddingRight: 10 }}>{lastCol}</span>
      </div>
      {rows.map((r, i) => {
        const grouped = String(r.key).startsWith("All other") || (kind === "chain" && r.isIndie);
        ...
        return (
          <div key={r.key} onClick={() => go(r)} style={{ display: "flex", alignItems: "center", background: rowBg, borderBottom: i === rows.length - 1 ? "none" : "1px solid var(--border)", cursor: grouped ? "default" : "pointer" }}>
            ...
            <span style={{ ...cell, width: 34, ...col, paddingRight: 10 }}>{lastCol === "plc/ac" ? r.avgPlc.toFixed(1) : r.ros.toFixed(1)}</span>
          </div>
        );
      })}
    </div>
