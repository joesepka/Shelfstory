...
const PROFILES = {
  ...
  brewery: {
    ...
    rules: { altParentStyles: ["THC"], keyChains: ["BINNY"] },
...
export const DEFAULT_RULES = {
  hideTerritories: [],
  /* Styles that belong to the SECOND label whatever the row's own parent column says. Blind
     Corner's THC items are Torch's, and that was written into the account card as a literal
     `style_parent.includes("THC")` test -- a client fact with no home in the profile, which is
     how it ended up as code. A client with no such rule leaves this empty and the card simply
     trusts each row's own parent. */
  altParentStyles: [],
  /* Chains big enough that "what do your other stores carry?" is a real sell-in line. This was
     a /BINNY/i regex inside SellStory -- one client's biggest account written into code, so no
     other client could ever get that line. */
  keyChains: [],   // sales-rep territories hidden from the territory rail
};
