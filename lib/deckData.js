...
import { SNAP_LABEL, SNAP_ISO, T12_MONTHS, T12_YR, monthAxis } from "./snapshot.js";

const S = a => a.reduce((x, y) => x + y, 0);
const pct = (a, b) => b > 0 ? Math.round((a - b) / b * 100) : (a > 0 ? null : 0);
// exported so the slide renderer can title-case names without a second copy of the rule
export const clean = s => String(s || "").trim().split(/\s+/).map(w => w ? w[0].toUpperCase() + w.slice(1).toLowerCase() : w).join(" ").replace(/(?<!['\w])\w/g, c => c.toUpperCase());
...
export function tfSpec(tf, thru) {
  ...
  const n = key === "30D" ? 1 : key === "90D" ? 3 : key === "L6M" ? 6 : Math.max(1, Math.min(12, snapMonth));
  const cur = [24 - n, 24];
  const cmpWin = cmp === "prev" ? [24 - 2 * n, 24 - n] : [12 - n, 12];
  const L = {
    /* THE LAST 30 DAYS (Joe, 2026-08-20). One window against the one before it. The labels say
       "30 days" and never "month", because these windows are exactly thirty days and drift --
       window 0 ends 8/14, the one before it 7/15 -- so calling either a month would be the same
       lie the axis used to tell. */
    "30D": { stat: "30-day cases", col: "30D", winShort: "30 days", winNoun: "last 30 days", period: `30 days ended ${thru || SNAP_LABEL}`,
             prevShort: "prior 30", prevLong: "the prior 30 days", rank: "ranked on the last 30 days", grew: "grew over the last 30 days", over: "over the last 30 days" },
  ...
  return {
    ...
    cmpNoun: yoy ? "same period last year" : (key === "30D" ? "prior 30 days" : key === "90D" ? "prior 90 days" : "prior 6 months"),
    rank: L.rank, grew: L.grew, over: L.over,
    newPhrase: yoy ? "new since last year" : key === "30D" ? "new in the last 30 days" : "new this quarter",
    zeroCmp: yoy ? "no volume at all in the same period last year" : key === "L6M" ? "no volume at all in the prior 6 months" : key === "30D" ? "no volume at all in the prior 30 days" : "no prior-quarter volume at all",
...
export function buildDeck({ scope, accts, acctMo, pgrid, dataThru, tf, thru, itemWin, styleOf }) {
  ...
  /* CITIES RANKED THE WAY BRANDS ARE (Joe, 2026-08-19: "show it top cities. so we can show
     variation with ranks"). Deliberately the SAME SHAPE as brandCut so the ranking slide can
     read it with no new builder and no new maths -- rows sorted by volume, a total, a prior
     total, a percent.

     This is exact, not apportioned: acctMo is 24 months per ACCOUNT and city is an account
     attribute, so an account belongs wholly to one city and nothing has to be split to get
     here. Cases follow the chosen timeframe; l90 and the account counts stay 90-day because
     they are the rate-of-sale inputs -- the same division of labour brandCut uses. */
  const cityCut = () => {
    const m = {};
    for (const id of withHist) {
      const a = A[id], mo = acctMo[id];
      const k = clean((a && a.city) || "") || "Unknown";
      const e = m[k] || (m[k] = { n: k, cur: 0, prev: 0, l90: 0, acc: 0, accP: 0 });
      const c = S(mo.slice(...T.cur)), v = S(mo.slice(...T.cmpWin)), l = S(mo.slice(-3));
      e.cur += c; e.prev += v; e.l90 += l;
      if (l > 0) e.acc++;
      if (v > 0) e.accP++;
    }
    const rows = Object.values(m)
      .map(e => ({ ...e, cur: Math.round(e.cur), prev: Math.round(e.prev), l90: Math.round(e.l90) }))
      .filter(e => e.cur > 0 || e.prev > 0).sort((a, b) => b.cur - a.cur);
    const tot = S(rows.map(r => r.cur)), totP = S(rows.map(r => r.prev));
    return { rows: rows.slice(0, 10), all: rows.length, tot, totP, pct: pct(tot, totP), mix: [] };
  };
  ...
  return {
    ...
    accSeries, rosSeries,
    draft: brandCut(true), pkg: brandCut(false), cities: cityCut(), styles: styleCut(),
    ...
    /* THE ACCOUNT SIDE, for slides that read accounts rather than products. acctMo[id] is 24
       months per ACCOUNT — which is what makes a monthly cut by territory or a per-account
       month grid possible at all, where itemWin (months per PRODUCT) cannot. Scope is already
       applied: acctRows holds only the accounts this deck is about. */
    acctRows: (accts || []).filter(a => ids.has(a.id)),
    acctMo: acctMo || null,
    snapIso: SNAP_ISO,
    ytdN: Math.max(1, Math.min(12, new Date(SNAP_ISO + "T00:00:00").getMonth() + 1)),
  };
}
...
/* THE DRILLABLE THREE (Joe, 2026-08-19: "Brand, Style, Item will be pretty universal").
   Draft/package stays available to the item slide as its own `pack` setting — it is a side of
   the book, not a thing you drill into. */
export const CUT_DIMS = [["brand", "Brand"], ["style", "Style"], ["item", "Item"]];
...
export function stackSeries(D, dim, maxBands = 4, span = 12) {
  ...
  const N = Math.max(3, Math.min(24, +span || 12));   // 24 windows of history are real
  const pkOf = {};
  ...
  for (const pk of Object.keys(pkOf)) {
    ...
    if (!w || !k || k === "—") continue;
    const a = m[k] || (m[k] = new Array(N).fill(0));
    for (let i = 0; i < N; i++) a[i] += +w[24 - N + i] || 0;   // the trailing N of the 24
  }
  ...
  if (bands.length > cap) {
    const rest = bands.slice(cap), o = new Array(N).fill(0);
    for (const b of rest) for (let i = 0; i < N; i++) o[i] += b.v[i];
    bands = bands.slice(0, cap).concat([{ k: `Other · ${rest.length}`, v: o, tot: o.reduce((x, y) => x + y, 0), other: true }]);
  }
  // the series is already exactly N long, so the axis is built to match rather than sliced
  const ax = monthAxis(N);
  const months = ax.months.map((mo, i) => mo + (ax.yr[i] ? " " + ax.yr[i] : ""));
