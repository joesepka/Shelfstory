...
import { profile , isBreweryShape } from "./profile";
...
export function loadMonthly(parent) {
  // SHAPE, NOT NAME -- a second brewery-shaped client got null, which is what left the
  // mobile header with no numbers at all (Joe, 2026-08-20).
  if (!isBreweryShape) return Promise.resolve(null);
...
export function healRows(rows, monthly) {
  ...
  return rows.filter(r => monthly[r.account_id]).map(r => {
    ...
    // windows since the last order, straight off the source line (0 = ordered this window,
    // 1 = 30-60d back, 2 = 60-90d back ...). Every no-order / quiet read hinges on THIS,
    // never on the pipeline's last_order columns (Joe's rule).
    let gapW = null;
    for (let i = m.length - 1; i >= 0; i--) { if ((m[i] || 0) > 0) { gapW = m.length - 1 - i; break; } }
