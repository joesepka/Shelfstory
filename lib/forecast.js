...
// SEASONAL MIRROR (Joe, 2026-08-16): the blend is level+trend, so a year whose volume
// lives in a BUMP (Oktoberfest, Marzen season, summer THC) forecasts flat and misses the
// recurrence. Detect concentration — the top-3-month share of last year's volume — and,
// when two years exist, require the bumps to land in roughly the same months (cosine of
// the two year-shapes). Returns a weight + last year's monthly shape scaled by the
// year-over-year level, or null when the series doesn't look seasonal. Guards: needs a
// real season (2+ selling months last year), so a just-launched item's single hot month
// never reads as seasonality.
function seasonalMirror(history) {
  if (!history || history.length < 24) return null;
  // A season needs evidence from BOTH years. A line that launched inside the last twelve
  // months is concentrated by definition — the mirror was reading Torch's March launch as
  // an annual bump and forecasting Sep-Dec at zero with a 587-case "season" next March
  // (Joe, 2026-08-17). No first-year sales -> no seasonality claim, full stop. Genuinely
  // seasonal lines (Autumn Standard's fall bump) have year-one sales and pass untouched.
  const first = history.findIndex(v => (v || 0) > 0);
  if (first < 0 || first >= 12) return null;
  const y1 = history.slice(0, 12), y2 = history.slice(12);
  const t2 = y2.reduce((a, b) => a + (b || 0), 0);
  if (t2 <= 0) return null;
  if (y2.filter(v => (v || 0) > 0).length < 2) return null;
  const sorted = y2.slice().sort((a, b) => (b || 0) - (a || 0));
  const s3 = ((sorted[0] || 0) + (sorted[1] || 0) + (sorted[2] || 0)) / t2;   // top-3-month share
  let w = Math.max(0, Math.min(1, (s3 - 0.38) / 0.3));   // flat year (~.25) -> 0; hard bump (>=.68) -> 1
  if (w <= 0) return null;
  const t1 = y1.reduce((a, b) => a + (b || 0), 0);
  if (t1 > 0) {
    let dot = 0, n1 = 0, n2 = 0;
    for (let i = 0; i < 12; i++) { const a = (y1[i] || 0) / t1, b = (y2[i] || 0) / t2; dot += a * b; n1 += a * a; n2 += b * b; }
    const align = dot / (Math.sqrt(n1 * n2) || 1);
    if (align < 0.35) return null;                       // bumps in different months = not a season
    w *= Math.max(0, Math.min(1, (align - 0.35) / 0.4));
  }
  if (w <= 0.05) return null;
  const scale = t1 > 0 ? Math.max(0.25, Math.min(4, t2 / t1)) : 1;   // carry yoy growth into the mirror
  return { w, mirror: y2.map(v => Math.max(0, (v || 0) * scale)) };  // forecast month k mirrors the same month last year
}
...
function _blend(history, key, { phi }) {
  ...
  // pull toward last year's monthly shape when the series reads seasonal (soft blend —
  // "sees past bumps and tries to mirror them"; the texture below rides the new shape)
  const sm = (!dead && !collapsing) ? seasonalMirror(history) : null;
  if (sm) base = base.map((v, k) => Math.max(0, v * (1 - sm.w) + sm.mirror[k] * sm.w));
...
export function autoForecast(history, key = "auto") {
  ...
  // A strongly seasonal series overrides the quiet-now and spike rules: a dormant
  // Oktoberfest in June still forecasts its fall bump, and a season ramping right now
  // isn't flattened as a one-month fluke. Softer seasonality falls through and gets the
  // gentle blend inside _blend instead.
  const smA = seasonalMirror(history);
  if (smA && smA.w >= 0.5) return smA.mirror.slice();
...
/* THE SHAPE DECIDES THE PATH, THE TARGET DECIDES THE END (Joe, 2026-08-19). The glide used to
   be linear whatever shape was on the node, so "grow then level" and "hockey stick" reached the
   number the same way. `ease` bends the path without moving either endpoint: it is still 0 at
   `start` and exactly 1 at `end`, so whatever number you asked for is what you land on.
     < 1  front-loaded — climbs early and levels off (grow-then-plateau)
     > 1  back-loaded  — holds low and arrives late (hockey stick)                            */
function ovFactor(ov, h, ease) {
  if (!ov || !ov.target) return 1;
  const start = ov.start || 0, end = ov.end != null ? ov.end : HORIZON, m = h + 1;
  const lin = m <= start ? 0 : m >= end ? 1 : (m - start) / Math.max(1, end - start);
  const frac = ease && ease !== 1 ? Math.pow(lin, ease) : lin;
  return Math.max(0, 1 + ov.target * frac);
}
// how each archetype wants the glide to travel — endpoints untouched, only the road between
const EASE_OF = { explosive: 1.9, hump: 1.4, climb: 1, maintain: 1, flat: 1, plateau: 0.55, creep: 0.7,
                  gdecline: 1.2, sharp: 0.6, dstab: 0.5 };
...
function compute(n, overrides, lens, assumptions, opts, pf) {
  ...
  const ease = EASE_OF[(assumptions[n.key] || {}).archetype] || 1;
  const eff = Array.from({ length: HORIZON }, (_, h) => (pf ? pf[h] : 1) * ovFactor(overrides[n.key], h, ease) * ovFactor(lens[n.key], h, ease) * av[h]);
  if (n.children.size === 0) {
    ...
    /* RECENCY LIFT (Joe, 2026-08-20). The level anchors on the mean of the last three windows so
       one fat order can't set it. But that also means a line genuinely stepping up is held back
       by the two months before the step. His rule, and it is the right one: lean on the newest
       window ONLY when distribution climbed alongside it. More shelves is a reason to believe
       the level actually moved; the same shelves ordering more is probably timing, and timing
       is exactly what the mean is there to absorb.

       Same shape as the damp above -- two independent signals have to agree before we move off
       the conservative read. The factor only ever lifts, never cuts. */
    const lift = typeof opts.levelLift === "function" ? (opts.levelLift(n) ?? 1) : 1;
    const k = la * lift;
    n.forecast = r.forecast.map((v, h) => Math.max(0, v * eff[h] * k));
    n.half = r.half.map((v, h) => v * eff[h] * k); n.conf = r.conf; n.cv = r.cv; n.collapsing = !!r.collapsing;
  } else {
    ...
    if (n.kind !== "root" && !overrides[n.key] && !lens[n.key]) {
      ...
      /* NO PRIOR YEAR IS NOT A FLAT YEAR (Joe, 2026-08-20: "this torch forecast seems way too
         conservative... it drops next month"). yoy used to fall back to 0 when there was no
         year-ago volume, and 0 sails through the "not growing" test below — so a line that
         launched THIS year, with nothing to decline from, was read as flat and capped like a
         fading book. Torch: L52 3,110 grown from zero, recent quarter running 56% above its own
         average, and the reconciliation still scaled the whole subtree by 0.569 — 5,024 cases
         cut to 2,856. On screen that is a climbing line that falls off a cliff next month, and
         it is the single fastest way to lose someone's trust in the whole forecast.

         So yoy is null when it cannot be measured, and the declining cap needs a REAL reading.
         The cap still exists for what it was built for: a book with two years of history that
         is genuinely fading on both horizons. It just can't fire on a launch any more. */
      const g90 = q1 > 0 ? (sum(hw.slice(-3)) - q1) / q1 : 0;
      const yoy = y0 > 0 ? (sum(hw.slice(12)) - y0) / y0 : null;
      let cap = null;
      if (live < 0.12) cap = 0;                                                  // gone dark → no forecast
      else if (g90 < -0.05 && yoy != null && yoy < 0.05) cap = L52 * (1 + Math.max(-0.6, g90 * 0.6));  // declining on BOTH horizons → below L52, floored (hedge, not death-spiral)
