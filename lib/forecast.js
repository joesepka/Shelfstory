...
function mYoY(h, H) { const n = h.length; if (n < 24) return mRun(h, H); const last12 = sum(h.slice(n - 12)), prior12 = sum(h.slice(n - 24, n - 12)); if (prior12 <= 0) return mRun(h, H); const g = clamp(last12 / prior12 - 1, -0.6, 0.8), base = last12 / 12; const out = []; for (let k = 1; k <= H; k++) out.push(Math.max(0, base * Math.pow(1 + g, k / 12))); return out; }
...
function _blend(history, key, { phi }) {
  ...
  const gr = (r, p) => p > 0 ? clamp((r - p) / p, -0.7, 0.8) : 0;
...
function compute(n, overrides, lens, assumptions, opts, pf) {
  ...
  if (n.children.size === 0) {
    ...
    n.forecast = r.forecast.map((v, h) => Math.max(0, v * eff[h]));
    n.half = r.half.map((v, h) => v * eff[h]); n.conf = r.conf; n.cv = r.cv; n.collapsing = !!r.collapsing;
  } else {
