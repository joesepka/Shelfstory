...
function _blend(history, key, { phi }) {
  ...
  const gr = (r, p) => p > 0 ? clamp((r - p) / p, -0.6, 0.4) : 0;   // cooled upside (was 0.8), matches shelfcast
...
export function autoForecast(history, key = "auto") {
  ...
  // Joe's rule: nothing sold in the last 90 days -> nothing forecast. Kept identical to the
  // desktop's lib/forecast.js — the two apps must never project differently.
  ...
  // never a single spike, can raise it. Kept identical to the desktop's lib/forecast.js.
