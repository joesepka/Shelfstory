...
export async function deckToPptx(pptxgen, D, meta = {}) {
  ...
  if (want(1)) {
    ...
    /* THE EXPORT NAMED THE WRONG COMPANY (Joe, 2026-08-20). DeckView already passes the
       right brand in `meta.universe`; this line ignored it and printed a constant, so the
       deck on screen said Northwind and the downloaded .pptx said Blind Corner. */
    T(s, `Prepared from distributor depletion reporting through the snapshot date   ·   ${(meta && meta.universe) || ""}`.trim().replace(/·\s*$/, "").trim(), 62, 660, 700, 18, { fontSize: pt(9), color: GREY2 });
  }
